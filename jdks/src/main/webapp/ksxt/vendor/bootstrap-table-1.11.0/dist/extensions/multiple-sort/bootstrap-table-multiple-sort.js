...
(function($) {
    ...
    var showSortModal = function(that) {
        ...
        if (!$(_id).hasClass("modal")) {
            ...
            sModal += '                     <button type="button" class="close" data-dismiss="modal" aria-label="关闭"><span aria-hidden="true">&times;</span></button>';
            ...
            sModal += '                                 <div id="toolbar" style="padding-bottom:5px;">';
            sModal += '                                     <button id="add" type="button" class="btn btn-default btn-sm"><i class="' + that.options.iconsPrefix + ' ' + that.options.icons.plus + '"></i> ' + that.options.formatAddLevel() + '</button>';
            sModal += '                                     <button id="delete" type="button" class="btn btn-default btn-sm" disabled><i class="' + that.options.iconsPrefix + ' ' + that.options.icons.minus + '"></i> ' + that.options.formatDeleteLevel() + '</button>';
            sModal += '                                 </div>';
            sModal += '                             </div>';
            sModal += '                         </div>';
            ...
            that.$sortModal.off('click', '.btn-primary').on('click', '.btn-primary', function() {
                ...
                that.options.sortPriority = $.map($rows, function(row) {
                    var $row = $(row),
                        ...
                        name = $row.find('.multi-sort-name').selectpicker('val'),
                        order = $row.find('.multi-sort-order').selectpicker('val');
                ...
                if (results.length > 0) {
                    if ($alert.length === 0) {
                        $alert = '<div class="alert alert-danger" role="alert">' + that.options.formatDuplicateAlertTitle() + ' ' + that.options.formatDuplicateAlertDescription() + '</div>';
    ...
    $.extend($.fn.bootstrapTable.locales, {
        ...
        formatDuplicateAlertTitle: function() {
            return '发现重复的排序字段！'; // Duplicate(s) detected!
        },
        formatDuplicateAlertDescription: function() {
            return '请删除或修改重复的排序字段'; // Please remove or change any duplicate column.
        },
    ...
    BootstrapTable.prototype.initToolbar = function() {
        ...
        if (this.options.showMultiSort) {
            ...
            if (!$multiSortBtn.length) {
                ...
                $multiSortBtn = '  <button class="multi-sort btn btn-default' + (this.options.iconSize === undefined ? '' : ' btn-' + this.options.iconSize) + '" type="button" title="' + this.options.formatMultipleSort() + '" onclick="window.app.$broadcast(\'showMultiSortModal\',\'' + sortModalId + '\')">';
    ...
    BootstrapTable.prototype.addLevel = function(index, sortPriority) {
        ...
        this.$sortModal.find('tbody')
            .append($('<tr>')
                .append($('<td>').text(text))
                .append($('<td>').append($('<select class="form-control multi-sort-name selectpicker">')))
                .append($('<td>').append($('<select class="form-control multi-sort-order selectpicker">')))
            );
        ...
        $multiSortName.selectpicker({
            style: 'btn-default btn-sm',
            size: 7,
            multipleSeparator: ','
        })
        $multiSortOrder.selectpicker({
            style: 'btn-default btn-sm',
            size: 7,
            multipleSeparator: ','
        })

        // $('.multi-sort-name,.multi-sort-order').selectpicker('refresh')
        this.$sortModal.find('.dropdown-toggle').dropdown()
    };
